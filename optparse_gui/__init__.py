...
def check_savefile(option, opt, value):
    ...
    if os.path.exists(value) and not os.path.isfile(value):
        raise optparse.OptionValueError(
            "option %s: Can't overwrite file: %s" % (opt, value))
    if option.filetypes:
        match = False
        for name,glb in option.filetypes:
            for glbi in glb.split(';'):
...
class Option(optparse.Option):
    ...
    TYPES = optparse.Option.TYPES + ("password","file","savefile", "files","multichoice")
...
class ProgressText(Progress):
    ...
    def deltaformat(delta):
        sd = list(map(float,str(delta).split(',',1)[-1].split(':')))
    ...
    def initprogressbar(self,message):
        ...
        print("%-*s->|"%(self.maxwidth-3,
                                        message[:self.maxwidth-3]), file=self.handle)
        self.handle.flush()
        self.barpos = 0
